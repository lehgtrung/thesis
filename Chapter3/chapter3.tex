...
Kiến trúc Bộ mã hóa-Bộ giải mã như tên gọi, gồm có 2 bộ phận chính là bộ mã hóa và bộ giải mã, nó có sử dụng 2 mô hình LSTM. Trong đó, mỗi mô hình LSTM được đảm nhiệm một vị trí Bộ mã hóa hoặc Bộ giải mã. Cụ thể:
\begin{itemize}
	\item Bộ mã hóa: là một Bi-LSTM. Đầu vào là một câu ở ngôn ngữ nguồn $x = \{x_0, ..., x_{S-1}\}$. Đầu ra là một véc-tơ có kích thước cố định, ở đây là véc-tơ trạng thái ẩn cuối cùng của Bi-LSTM của bộ mã hóa $h_{S-1}$. Nhiệm vụ của bộ mã hóa là mã hóa câu đầu vào thành một véc-tơ mà véc-tơ này chứa toàn bộ thông tin cần thiết để thực hiện việc dịch.
	\item Bộ giải mã: là một Uni-LSTM. 
\end{itemize}
