...
Cách đơn giản nhất để quyết định xem nên chọn từ nào từ phân phối xác suất trên bộ từ vựng $P(\bm{y} | y<t, x\})\  y \in V$ là chọn ra từ nào có xác suất cao nhất:
\begin{equation}
\hat{y} = \operatorname*{argmax}_{y_t} (p(y_t | y < t, x))
\end{equation}
Tuy nhiên, cách này trong thực tế cho ra kết quả không thực sự tốt. 
