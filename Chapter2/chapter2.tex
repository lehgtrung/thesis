...
Như chúng tôi đã đề cập trong chương 1, \textit{mô hình ngôn ngữ} (language model) là một bộ phận quan trọng trong cả dịch máy thống kê và dịch máy nơ-ron. Cụ thể hơn, mô hình ngôn ngữ là một phân phối xác suất trên một chuỗi các từ. Cho trước một chuỗi $w_1,w_2,...,w_n$ (ký hiệu $w_{1:n}$), mô hình ngôn ngữ gán cho nó một xác suất $P(w_{1:n})$ đại diện cho độ "trơn tru" của chuỗi đó. Ta có thể mô phỏng xác suất này bằng cách sử dụng quy tắc dây chuyền trong xác suất:
\begin{equation} \label{lmGeneral1}
	P(w_{1:n}) = P(w_1)P(w_2|w_1)P(w_3|w_{1:2})...P(w_n|w_{1:n-1}) = \prod_{t=1}^{n} P(w_t|w_{1:t-1})
\end{equation}

Trong bài toán dịch máy, mô hình ngôn ngữ được sử dụng để tìm từ tiếp theo trong một chuỗi các từ đã dịch được. Gọi $P(w_n|w_{1:n-1})$ là xác suất một từ $w_n$ xuất hiện theo sau một chuỗi $w_1,w_2,...,w_{n-1}$, xác suất này có thể được tính như sau:
\begin{equation} \label{lmGeneral2}
	P(w|w_{1:n}) = \frac{P(w_{1:n})}{P(w_{1:n-1})}
\end{equation}
