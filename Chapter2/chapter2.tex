...
%bằng cách áp dụng định nghĩa $\tau$ lần. Ví dụ với thời điểm $\tau = 3$, chúng ta có:

%\begin{equation} \label{basicRnnEquationUnrolled1}
%	\begin{split}
%  		A {}  & = f \left(h_2, x_t \right) \\
%        	  & = f \left(f \left(h_1, x_t \right), x_t \right) \\
%        	  & = f \left(f \left(f \left(h_0, x_t \right), x_t \right), x_t %\right)
%	\end{split}
%\end{equation}
