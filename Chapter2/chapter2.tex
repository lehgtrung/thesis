...
\subsection{LSTM xếp chồng (Stacked LSTM)}

\subsection{LSTM hai chiều (Bidirectional LSTM)}
...
Công thức \ref{markovAssumption} theo giả định markov trở thành:
\begin{equation} \label{traditionalLM}
...
\begin{equation} \label{ngramLMWithSmoothing}
	p_{add-k} \left(w_{i+1}=m|w_{i-k:i} \right) = \frac{\# \left(w_{i-(n-1):i+1} \right) + \alpha}{\# \left(w_{i-(n-1):i} \right) + \alpha \left|V \right| }
\end{equation}
